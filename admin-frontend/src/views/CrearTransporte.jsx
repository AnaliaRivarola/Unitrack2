...
export const CrearTransporte = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!gpsSeleccionado) {
        setError("Debe seleccionar un GPS para el transporte.");
        return;
      }

      // Verificar si el GPS ya está asociado a otro transporte
      const token = localStorage.getItem("admin_token");
      const response = await axios.get(`http://localhost:5000/api/transportes/gps/${gpsSeleccionado}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      if (response.data.transporte) {
        setError(`El GPS seleccionado ya está asociado al transporte "${response.data.transporte.nombre}".`);
        return;
      }
      ...
      setError("Hubo un error al crear el transporte.");
    }
  };
